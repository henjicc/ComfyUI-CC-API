...
- **voice** (选择): 选择语音合成的音色，支持多种语言的不同风格音色。节点会显示友好的音色名称，系统会自动适配对应的voice_type和resource_id。
...
豆包TTS节点支持多种音色，包括：
- 豆包语音合成模型2.0音色（如vivi、小何、云舟、小天等）
- 豆包语音合成模型1.0音色（包括多情感音色和普通音色）

在节点中选择音色时，会显示友好的音色名称（如"爽快思思（多情感）"、"冷酷哥哥（多情感）"等），系统会自动适配对应的voice_type和resource_id，无需手动选择。
