...
class MinimaxHailuoPPIONode:
    """Minimax Hailuo-02 视频生成节点 (派欧云)"""
    ...
    def call_minimax_hailuo_api(self, api_key, prompt, image, end_image, duration, resolution, enable_prompt_expansion, seed):
        """调用Minimax Hailuo-02 API"""
        try:
            # 准备请求数据
            payload = {
                "prompt": prompt,
            ...
            raise Exception(f"Error calling Minimax Hailuo-02 API: {str(e)}")
    ...
    def generate_video(
        ...
        try:
            # 调用API生成视频
            print("Calling Minimax Hailuo-02 API...")
            task_id = self.call_minimax_hailuo_api(
                api_key=api_key,
                prompt=prompt,
                image=image,
                end_image=end_image,
...
# Node class mappings
NODE_CLASS_MAPPINGS = {
    "MinimaxHailuoPPIONode": MinimaxHailuoPPIONode
}

# Node display name mappings
NODE_DISPLAY_NAME_MAPPINGS = {
    "MinimaxHailuoPPIONode": "Minimax Hailuo-02 (派欧云)"
}