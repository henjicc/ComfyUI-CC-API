...
class KlingPPIOImg2VideoNode:
    """Kling V2.5 Turbo 图生视频节点 (派欧云)"""
    ...
    def call_kling_img2video_api(self, api_key, image, prompt, duration, cfg_scale, mode, seed, negative_prompt=None):
        """调用Kling V2.5 Turbo图生视频API"""
        try:
            # 准备请求数据
            payload = {
                "prompt": prompt,
            ...
            response = requests.post(
                "https://api.ppinfra.com/v3/async/kling-2.5-turbo-i2v",
                headers=headers,
                json=payload,
                timeout=30
            )
            ...
            raise Exception(f"Error calling Kling Img2Video API: {str(e)}")
    ...
    def generate_video(
        ...
        try:
            # 调用API生成视频
            print("Calling Kling V2.5 Turbo Img2Video API...")
            task_id = self.call_kling_img2video_api(
                api_key=api_key,
                image=image,
                prompt=prompt,
                duration=duration,
...
class KlingPPIOText2VideoNode:
    """Kling V2.5 Turbo 文生视频节点 (派欧云)"""
    ...
    def call_kling_text2video_api(self, api_key, prompt, duration, aspect_ratio, cfg_scale, mode, seed, negative_prompt=None):
        """调用Kling V2.5 Turbo文生视频API"""
        try:
            # 准备请求数据
            payload = {
                "prompt": prompt,
            ...
            response = requests.post(
                "https://api.ppinfra.com/v3/async/kling-2.5-turbo-t2v",
                headers=headers,
                json=payload,
                timeout=30
            )
            ...
            raise Exception(f"Error calling Kling Text2Video API: {str(e)}")
    ...
    def generate_video(
        ...
        try:
            # 调用API生成视频
            print("Calling Kling V2.5 Turbo Text2Video API...")
            task_id = self.call_kling_text2video_api(
                api_key=api_key,
                prompt=prompt,
                duration=duration,
                aspect_ratio=aspect_ratio,
...
# Node class mappings
NODE_CLASS_MAPPINGS = {
    "KlingPPIOImg2VideoNode": KlingPPIOImg2VideoNode,
    "KlingPPIOText2VideoNode": KlingPPIOText2VideoNode
}

# Node display name mappings
NODE_DISPLAY_NAME_MAPPINGS = {
    "KlingPPIOImg2VideoNode": "可灵 2.5 图生视频 (派欧云)",
    "KlingPPIOText2VideoNode": "可灵 2.5 文生视频 (派欧云)"
}